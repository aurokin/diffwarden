...
maps `minimal` to `low` and `xhigh` to `max` on the CLI transport (the SDK transport resolves
`xhigh`/`max` against the model catalog at preflight); Droid and Grok map `minimal` to `low`;
engines without a distinct `max` level (Codex, Droid, Grok, Copilot, Pi) treat `max` as `xhigh`;
Codex, Pi, OpenCode, Gemini, and Cursor otherwise record exact requested values where those
